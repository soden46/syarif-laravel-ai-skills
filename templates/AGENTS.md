...
Use the skills from this repository when working on Laravel applications.
...
- `controller-cleanup`: Use for thin controllers and route boundaries.
- `form-requests`: Use for validation and request authorization.
...
Read the relevant `SKILL.md` first. Load only the referenced files needed for the current task. User instructions and project-local rules override these reusable standards. For the complete mapped catalog, see `docs/SUPERPOWERS_SKILL_MAPPING.md`.
...
Gunakan skill dari repository ini ketika mengerjakan aplikasi Laravel.
...
- `controller-cleanup`: Pakai untuk controller tipis dan route boundary.
- `form-requests`: Pakai untuk validasi dan request authorization.
...
Baca `SKILL.md` yang relevan terlebih dahulu. Load hanya file referensi yang dibutuhkan untuk task saat ini. Instruksi user dan aturan lokal project mengalahkan standar reusable ini. Untuk katalog mapping lengkap, lihat `docs/SUPERPOWERS_SKILL_MAPPING.md`.
