...
description: Build and refine Laravel UI/UX/frontends with stack-aware implementation, backend contract alignment, Playwright tests, and agent-browser inspection.
tags:
  - laravel
  - php
  - ui
...
2. Read the backend surface that the UI must connect to: routes, controllers, Form Requests, policies, API Resources, DTOs, Inertia props, Livewire public state/actions, model relationships, events, files, and pagination.
3. Define the UI contract before styling: data needed, user actions, request payloads, validation errors, authorization-dependent controls, loading states, empty states, success states, and failure states.
4. Design to the detected stack and project conventions. Reuse existing layouts, tokens, components, tables, forms, modals, navigation, and icons before introducing a new visual language.
5. Implement the UI in the target stack, wired to real backend routes, Livewire actions, Inertia props, or API clients. Avoid static-only screens unless the requested artifact is explicitly a mockup.
6. Inspect the result in a real browser with realistic data, screenshots, console output, interaction states, and at least mobile and desktop viewports.
7. Iterate on hierarchy, spacing, alignment, text fit, color, controls, and backend state mapping until the interface feels intentional and works through the real workflow.
8. Use `responsive-ui-testing` for the final viewport matrix when responsiveness, overflow, clipping, modals, tables, or visual regression matters.
...
- Use Playwright for durable tests, assertions, auth setup, deterministic seeds, screenshots in CI, regression coverage, and repeated workflows.
...
- Playwright coverage exists for high-value or regression-prone flows when the project supports it;
