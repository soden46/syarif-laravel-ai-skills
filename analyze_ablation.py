...
def read_grading(base, iter_name, case, skill_type='with_skill'):
    g_path = os.path.join(base, iter_name, f'eval-{case}', skill_type, 'grading.json')
...
for case in cases:
    ...
    v3_base_scores = []
    for it in v3_iters:
        g_ws = read_grading(v3_base, it, case, 'with_skill')
        g_base = read_grading(v3_base, it, case, 'without_skill')
    ...
    a_base_scores = []
    for it in a_iters:
        g_ws = read_grading(a_base, it, case, 'with_skill')
        g_base = read_grading(a_base, it, case, 'without_skill')
        a_ws_scores.append(get_pass_rate(g_ws))
        a_base_scores.append(get_pass_rate(g_base))
    
    v3_ws = sum(v3_ws_scores)/len(v3_ws_scores) if v3_ws_scores else 0
    v3_base = sum(v3_base_scores)/len(v3_base_scores) if v3_base_scores else 0
    a_ws = sum(a_ws_scores)/len(a_ws_scores) if a_ws_scores else 0
    a_base = sum(a_base_scores)/len(a_base_scores) if a_base_scores else 0
    ...
    case_results.append((case, v3_ws, a_ws, delta, v3_base, a_base, v3_adv, a_adv, adv_delta))
    
    print(f"{case:<35} {v3_ws:>7.3f} {a_ws:>7.3f} {delta:>+7.3f} {v3_base:>8.3f} {a_base:>8.3f} {v3_adv:>7.3f} {a_adv:>7.3f} {adv_delta:>+7.3f}")
